defmodule ProduceWeb.GroceryLive.FormComponent do
  use ProduceWeb, :live_component
  ...
  def update(%{grocery: grocery} = assigns, socket) do
    changeset = FoodStorage.change_grocery(grocery)
  ...
  def handle_event("validate", %{"grocery" => grocery_params}, socket) do
    changeset =
      ...
      |> FoodStorage.change_grocery(grocery_params)
  ...
  defp save_grocery(socket, :edit, grocery_params) do
    case FoodStorage.update_grocery(socket.assigns.grocery, grocery_params) do
      {:ok, grocery} ->
  ...
  defp save_grocery(socket, :new, grocery_params) do
    case FoodStorage.create_grocery(grocery_params) do
      {:ok, grocery} ->
        notify_parent({:saved, grocery})
